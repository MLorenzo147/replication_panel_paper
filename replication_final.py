...
def load_data(path: Path) -> pd.DataFrame:
    ...
    required = [COUNTRY_COL, YEAR_COL, RAW_GDP, RAW_CPI, RAW_ENERGY,
                RAW_GDPNOM, RAW_EXPORTS, RAW_IMPORTS, RAW_EXPENDITURE, RAW_INVEST]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Colonnes manquantes : {missing}")

    num_cols = [YEAR_COL, RAW_GDP, RAW_CPI, RAW_ENERGY, RAW_GDPNOM,
...
def load_table56(path: Path) -> Optional[pd.DataFrame]:
    ...
    if exclude_col:
        out["exclude"] = raw[exclude_col].astype(str).str.strip().str.lower().isin(["1", "true", "yes", "y"])
...
def compute_ecm(df: pd.DataFrame) -> Tuple[pd.DataFrame, sm.regression.linear_model.RegressionResultsWrapper]:
    df = df.copy()
    y = df[LOG_GDP]
    X = sm.add_constant(df[[LOG_CPI, LOG_ENERGY, OPEN_TRADE, GOV_EXP, INVEST]], has_constant="add")
...
def panel_variance_decomp(df: pd.DataFrame, variables: Iterable[str]) -> pd.DataFrame:
    ...
    for var in variables:
        ...
        rows.append({
            "variable": var, "N": n_entities, "NT": nt,
            "NT_over_N": nt / n_entities if n_entities else np.nan,
            "var_total": overall_var, "var_between": between_var,
            "var_within": within_var, "var_twfe": twfe_var,
            "var_fd": fd_var, "pct_within": within_pct,
        })
...
def observation_tables(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    return {
        ...
        "matrix":     df_r.pivot_table(index=COUNTRY_COL, columns=YEAR_COL,
                                        values=LOG_GDP, aggfunc="size"),
    }
...
def _cips_manual(series: pd.Series, lags: int = 1) -> Tuple[float, float]:
    ...
    ybar_by_year = df[[YEAR_COL, "ybar"]].drop_duplicates().sort_values(YEAR_COL).set_index(YEAR_COL)["ybar"]
...
def estimate_static_panel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fd   = FirstDifferenceOLS.from_formula(
        "lrgdpmad ~ lcpi + lenpr + open + expgdp + iy", data=df).fit()
    return _results_to_table({"between": between, "within": within,
                               "re_mundlak": re_mundlak, "twfe": twfe, "fd": fd})
...
def _results_to_table(results: Dict[str, object]) -> pd.DataFrame:
    ...
    for model_name, res in results.items():
        for var in res.params.index:
            rows.append({"model": model_name, "variable": var,
                         "coef": res.params[var],    "se": res.std_errors[var],
                         "t":    res.tstats[var],     "p":  res.pvalues[var]})
...
def fit_dynamic_ols(df: pd.DataFrame) -> sm.regression.linear_model.RegressionResultsWrapper:
    y = df["dlrgdpmad"]
    X = sm.add_constant(df[["l_dlrgdpmad","dlenpr","l_dlenpr",
                              "dlcpi2","dopen","dexpgdp","diy","l_ecterm"]],
                        has_constant="add")
...
def fit_dynamic_iv(df: pd.DataFrame) -> IV2SLS:
    ...
    exog    = sm.add_constant(df[["l_dlenpr","dlcpi2","dopen","dexpgdp","diy","l_ecterm"]],
                              has_constant="add")
...
def irf_standard_errors(beta1: float, beta2: float, rho: float,
                         se: Dict[str, float]) -> List[float]:
    ...
    for _ in range(3, 5):
        out.append(np.sqrt((rho*out[-1])**2 + (se_rho*tau_prev)**2 + (rho*se_b2)**2))
...
def infer_instruments(df: pd.DataFrame) -> List[str]:
    ...
    return [c for c in df.columns if any(p in c.upper() for p in
            ["OPEC","US","SHOCK","SUPPLY","INSTR","IV_"])]
...
def estimate_ccemg(
    ...
    exog_vars: List[str], endog_vars: List[str], instruments: List[str],
    exog_only: bool = False,
    cs_mean_vars: Optional[List[str]] = None,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    vars_for_means = cs_mean_vars if cs_mean_vars is not None else [dep] + exog_vars + endog_vars
    ...
    for country, g in df.groupby(level=0):
        ...
        if exog_only or not instruments:
            ...
            res     = IV2SLS(y_reg,
                             sm.add_constant(X_reg, has_constant="add"),
                             E_reg, Z_reg).fit(
                        cov_type="kernel", kernel="bartlett", bandwidth=1)
            params, se = res.params, res.std_errors

        for var in params.index:
            rows.append({"country": country, "variable": var,
                         "coef": params[var], "se": se[var]})
...
def _mg_from_country(country_df: pd.DataFrame, robust: bool = False) -> pd.DataFrame:
    ...
    for var, g2 in country_df.groupby("variable"):
        ...
        if robust:
            ...
            se_mg = float(np.std(vals, ddof=1) / np.sqrt(vals.size)) if vals.size > 1 else np.nan
...
def build_robustness_table(
    df_base: pd.DataFrame, df_full: pd.DataFrame, dep: str,
    exog_vars: List[str], endog_vars: List[str], instruments: List[str],
) -> pd.DataFrame:
    variants = {}

    _, variants["inst"]     = estimate_ccemg(df_base, dep, exog_vars, endog_vars, instruments, False, CORE_VARS)
    _, variants["exog"]     = estimate_ccemg(df_base, dep, exog_vars, endog_vars, instruments, True, CORE_VARS)

    exog_womod = [v for v in exog_vars if v not in ("mod", "dlenprmod")]
    _, variants["womod"]    = estimate_ccemg(df_base, dep, exog_womod, endog_vars, instruments, False, CORE_VARS)

    df_rec = df_base.copy()
    df_rec["recession_2009"] = (df_rec.index.get_level_values(YEAR_COL) == 2009).astype(int)
    _, variants["recession"] = estimate_ccemg(df_rec, dep, exog_vars + ["recession_2009"],
                                               endog_vars, instruments, False, CORE_VARS)
    ...
    _, variants["gerire"]   = estimate_ccemg(df_gerire[mask], dep, exog_vars, endog_vars, instruments, False, CORE_VARS)
    _, variants["sixties"]  = estimate_ccemg(df_full,  dep, exog_vars, endog_vars, instruments, False, CORE_VARS)

    rows = []
    for name, tbl in variants.items():
        for _, row in tbl.iterrows():
            rows.append({"variant": name, "variable": row["variable"],
...
def _render_table_png(tbl: pd.DataFrame, title: str, notes: str,
                      filepath: Path, fig_w: float = 8.5, row_h: float = 0.32,
    ...
    ax.axis("off")
    t = ax.table(cellText=tbl.values.tolist(),
...
def export_table1(df: pd.DataFrame, out_dir: Path) -> None:
    varmap = {
        LOG_GDP: "GDP",       LOG_CPI: "CPI",        LOG_ENERGY: "ENERGY",
        OPEN_TRADE: "OpenTrade", GOV_EXP: "GovExp",  INVEST: "Invest",
        f"d{LOG_GDP}": "D.GDP",  f"d{LOG_CPI}": "D.CPI",
        f"d{LOG_ENERGY}": "D.ENERGY",
        f"d{OPEN_TRADE}": "D.OpenTrade", f"d{GOV_EXP}": "D.GovExp",
        f"d{INVEST}":     "D.Invest",
    }
    df_r  = df.reset_index()
    rows  = []
    ...
    _render_table_png(
        ...
        title="Table 1 – Data Summary",
         notes=("Notes: Real GDP (GDP), CPI and Energy Prices (ENERGY) are in logarithms. "
...
def export_table2(cips_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    for var in varmap:
        ...
        lag_map = {
            int(r["lag"]): (r["stat_level"], r["pvalue_level"], r["stat_diff"], r["pvalue_diff"])
            for _, r in sub.iterrows()
        }

        lvl = {k: lag_map.get(k, (np.nan, np.nan, np.nan, np.nan))[0:2] for k in range(4)}
        dif = {k: lag_map.get(k, (np.nan, np.nan, np.nan, np.nan))[2:4] for k in range(4)}
    ...
    def extract(mg):
        ...
        for code, label in VAR_ORDER:
            ...
            if r.empty:
                ...
                out[label] = _fmt_coef(r.iloc[0]["coef"], r.iloc[0]["se"], r.iloc[0]["p"])
        return out
...
def export_table4(robustness_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    COL_LABELS = {"inst": "inst", "exog": "exog", "womod": "w/o_mod",
    ...
    # Observations (approx)
    obs_map = {"inst":"808","exog":"808","w/o_mod":"808",
               "recession":"808","ger_ire":"808","sixties":"976"}
...
def export_table6(
    ...
    if intensity_df is not None and not intensity_df.empty:
        ...
        for c in countries:
            ...
            if "intensity" in tmp.columns:
                if (tmp["_key"] == key).any():
                    ...
                    row["intensity"] = tmp.loc[tmp["_key"] == name_key, "intensity"].iloc[0]
                else:
                    row["intensity"] = np.nan
            if "exclude" in tmp.columns:
                if (tmp["_key"] == key).any():
                    row["exclude"] = bool(tmp.loc[tmp["_key"] == key, "exclude"].iloc[0])
                elif (tmp["_key"] == name_key).any():
                    row["exclude"] = bool(tmp.loc[tmp["_key"] == name_key, "exclude"].iloc[0])
                else:
                    row["exclude"] = False
            out.append(row)
        ...
        def _drop_outliers(df: pd.DataFrame, period: str, n: int = 2) -> pd.DataFrame:
            ...
            outliers = sub.assign(resid=res.resid).sort_values("resid", key=lambda s: s.abs()).tail(n)
        ...
        tbl = pd.DataFrame([
            {"Specification": "(1) Pre-1983 ~ Intensity",  "Coef":"n/a","t-stat":"n/a","N":18,"F":"n/a","RMSE":"n/a"},
            {"Specification": "(2) Post-1982 ~ Intensity", "Coef":"n/a","t-stat":"n/a","N":18,"F":"n/a","RMSE":"n/a"},
            {"Specification": "(3) Pre-1983 (excl. outliers)",  "Coef":"n/a","t-stat":"n/a","N":16,"F":"n/a","RMSE":"n/a"},
            {"Specification": "(4) Post-1982 (excl. outliers)", "Coef":"n/a","t-stat":"n/a","N":16,"F":"n/a","RMSE":"n/a"},
        ])
...
def _country_grid(df: pd.DataFrame, ycol: str, ylabel: str,
                  suptitle: str, figname: str, out_dir: Path,
...
def export_fig_a7(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    for idx, c in enumerate(countries[:20]):
        ax  = axes_flat[idx]
        sub = df_r[df_r[COUNTRY_COL] == c].sort_values(YEAR_COL).dropna(subset=[f"d{LOG_GDP}"])
        ax.plot(sub[YEAR_COL], sub[f"d{LOG_GDP}"], lw=0.85, color="black", label="D.GDP")
        if "residual" in sub.columns:
            ax.plot(sub[YEAR_COL], sub["residual"], lw=0.7,
                    color="#777777", linestyle="--", label="Residual")
    ...
    handles = [
        ...
        plt.Line2D([0],[0], color="#777777", lw=0.8, linestyle="--", label="Residuals"),
    ]
    fig.legend(handles=handles, loc="lower right", fontsize=7, frameon=False)
    fig.text(0.5, 0.01, "Year", ha="center", fontsize=8)
    fig.suptitle("Fig. A.7 – Actual change in Real GDP and CCEMG residuals by country",
...
def run_all() -> None:
    ...
    for var in CORE_VARS:
        for lag in range(0, 4):
            ...
            cips_rows.append({"variable": var, "lag": lag,
    ...
    estimate_static_panel(df_full).to_csv(OUTPUT_DIR / "panel_models_table.csv", index=False)

    # ── 4) Modele dynamique ─────────────────────────────────────────────────
    dyn_ols = fit_dynamic_ols(df_full)
    dyn_iv  = fit_dynamic_iv(df_full)

    pd.DataFrame({"coef": dyn_ols.params, "se": dyn_ols.bse,
                  "t": dyn_ols.tvalues,   "p":  dyn_ols.pvalues}
                ).to_csv(OUTPUT_DIR / "ardl_ols.csv")
    pd.DataFrame({"coef": dyn_iv.params,      "se": dyn_iv.std_errors,
                  "t":    dyn_iv.tstats,       "p":  dyn_iv.pvalues}
                ).to_csv(OUTPUT_DIR / "ardl_iv.csv")

    try:
        pd.DataFrame([{"endog": n, "r2": s.rsquared}
                      for n, s in dyn_iv.first_stage.items()]
    ...
    pd.DataFrame([{"lt_coef": lt, "rho": rho}]).to_csv(OUTPUT_DIR / "long_term.csv", index=False)

    # ── 5) CCE-MG (Table 3) ────────────────────────────────────────────────
    cce_regs = [
        "dlcpi", "dlenpr", "dopen", "dexpgdp", "diy",
        "l_lrgdpmad", "l_lcpi", "l_open", "l_iy",
        "lrgdpmadT", "lcpiT", "lenprT", "openT", "expgdpT", "iyT",
    ]
    ...
    dep       = "dlrgdpmad"
    exog_vars = ["l_dlcpi2","dopen","dexpgdp","diy",
                 "l_lrgdpmad","l_lcpi","l_open","l_iy","mod","dlenprmod"]
    endog_vars= ["dlenpr"]

    cc_country, cc_mg = estimate_ccemg(
        df_base, dep, exog_vars, endog_vars, instruments,
        exog_only=not instruments, cs_mean_vars=CORE_VARS)
